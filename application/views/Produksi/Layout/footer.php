...
<script>
	<?php
	$data_analisis = $this->db->query("SELECT MONTH(tgl_tran) as bulan, YEAR(tgl_tran) as tahun, bahan_baku.id_bb, SUM(jml) as total FROM `tran_bb` JOIN detail_bb ON tran_bb.id_tran_bb=detail_bb.id_tran_bb JOIN bahan_baku ON bahan_baku.id_bb=detail_bb.id_bb WHERE bahan_baku.id_bb='$id_bb' GROUP BY MONTH(tgl_tran), bahan_baku.id_bb")->result();
	foreach ($data_analisis as $key => $value) {
		$forecast[] = $value->total;
		$thn[] = $value->bulan;
	}
	?>
	var ctx = document.getElementById('permintaan');
...
<script>
	<?php
	$transaksi = $this->db->query("SELECT SUM(tot_bayar) as total, tgl_tran FROM `tran_bb` GROUP BY tgl_tran")->result();
...
<script>
	...
	$("#bj").on('change', function() {
		$(".stok_bj").html($(this).find(':selected').attr('data-stok_bj'));
		$(".stok_bj").val($(this).find(':selected').attr('data-stok_bj'));
	});
</script>
<script>
...
<script>
	<?php
	$stok_bb = $this->db->query("SELECT SUM(sisa) as sisa_qty, nama_bb, keterangan, harga, MAX(tgl_tran) as tgl_tran  FROM `detail_bb` JOIN tran_bb ON tran_bb.id_tran_bb = detail_bb.id_tran_bb JOIN bahan_baku ON bahan_baku.id_bb=detail_bb.id_bb GROUP BY bahan_baku.id_bb")->result();
	foreach ($stok_bb as $key => $value) {
		$stok_bahan_baku[] = $value->sisa_qty;
		$nama_bb[] = $value->nama_bb;
	}
	?>
	var ctx = document.getElementById('stok_bb');
	var grafik = new Chart(ctx, {
		type: 'bar',
		data: {
			labels: <?= json_encode($nama_bb) ?>,
			datasets: [{
				label: 'Grafik Stok Bahan Baku Yg Tersedia',
				data: <?= json_encode($stok_bahan_baku) ?>,
				backgroundColor: [
					'rgba(201, 77, 201, 1)',
					'rgba(255, 207, 207, 1)',
					'rgba(255, 206, 86, 0.80)',
